...
(function () {
  ...
  async function init() {
    ...
    /*
     * OAuth may return to Site URL root (was xfreeze.com → now redirected to
     * freezestack.com/?code= or /login?code=). Finish login on any page.
     */
    if (isOAuthCallback() && session && session.user) {
      cleanAuthParamsFromUrl();
